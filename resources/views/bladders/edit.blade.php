                        ...
                        <form action="{{ route('bladders.update', $bladder) }}" method="POST" class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('BladderCode') is-invalid @enderror"
                                    ...
                                    value="{{ old('BladderCode', $bladder->BladderCode) }}" placeholder="Enter bladder code"
                                    required>
                                @error('BladderCode')
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('Status') is-invalid @enderror" id="Status"
                                    ...
                                    <option value="ready"
                                        {{ old('Status', $bladder->Status) == 'ready' ? 'selected' : '' }}>Ready
                                    </option>
                                    <option value="maintenance"
                                        {{ old('Status', $bladder->Status) == 'maintenance' ? 'selected' : '' }}>Maintenance
                                    </option>
                                    <option value="mounted"
                                        {{ old('Status', $bladder->Status) == 'mounted' ? 'selected' : '' }}>Mounted</option>
                                    <option value="test"
                                        {{ old('Status', $bladder->Status) == 'test' ? 'selected' : '' }}>Test
                                    </option>
                                </select>
                                @error('Status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-12">
                                <button type="submit" class="btn btn-primary px-5">
                                    <i class="bx bx-save me-1"></i>Update Bladder
                                </button>
